...
def derive_conditions_generic(readings, thresholds) -> dict:
    """
...
def derive_conditions_generic(readings, thresholds):
    """
...
def compose_paragraph_generic(readings, thresholds) -> str:
    """
    5문장 단락 (도메인 무관):
    ...
    if not readings:
        return ""
    cond = derive_conditions_generic(readings, thresholds)
...
def compose_essay_generic(readings, thresholds) -> str:
    """
    ...
    body = compose_paragraph_generic(readings, thresholds)
    if primary_action:
        conclusion = (
    ...
    for domain, readings in readings_by_domain.items():
        ...
        if readings:
            body_paragraphs.append(
                f"[{domain.upper()}] " + compose_paragraph_generic(readings, thresholds)
            )
            cond = derive_conditions_generic(readings, thresholds)
