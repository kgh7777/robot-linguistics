    ...
    for r in readings:
        for th in thresholds:
            if r.sensor_type != th.sensor_type:
                continue
            v, t = r.temperature_c, th.threshold_value
            triggered = False
...
def _declarative_clause_generic(readings) -> str:
    ...
    for r in readings:
        ...
        parts.append(f"the {r.sensor_type} is {r.temperature_c:.1f}{r.unit}")
    if not parts:
        return ""
...
def _conditional_clause_generic(readings, action) -> str:
    if not action or not readings:
        return ""
    r = readings[0]
    return f"If the {r.sensor_type} is {r.temperature_c:.1f}{r.unit}, {action}."
...
def derive_conditions_universal(
    ...
    for r in readings:
        for th in thresholds:
            if r.sensor_type != th.sensor_type:
                continue
            if _compare_generic(r.temperature_c, th.comparison, th.threshold_value):
...
def _format_reading_generic(r: SensorReading) -> str:
    ...
    return f"the {r.sensor_type} is {r.temperature_c:.1f}{r.unit}"
...
def declarative_clause_generic(readings: _List[SensorReading]) -> str:
    ...
    first = sorted_r[0]
    head = f"The {first.sensor_type} is {first.temperature_c:.1f}{first.unit}"
    if len(sorted_r) == 1:
        return head + "."
...
def condition_clause_generic(
    ...
    if r is None:
        return ""
    return f"If the {r.sensor_type} is {r.temperature_c:.1f}{r.unit},"
...
def _reading_field(r):
    ...
    if hasattr(r, "value") and hasattr(r, "unit") and hasattr(r, "sensor_type"):
        return r.temperature_c, r.unit, r.sensor_type
...
def direction_phrase(value, threshold_value, comparison, unit=""):
    """
    비교 방향에 따라 정확한 phrase를 반환.

    Args:
        value: 측정값
        threshold_value: 임계값
        comparison: ">=" 또는 "<=" 또는 ">" 또는 "<" 또는 "=="
        unit: 단위 (예: "V", "°C")
    ...
    if comparison == ">=":
        ...
        if value > threshold_value:
            ...
            return f"is at or below the threshold of {threshold_value}{unit}"
    elif comparison == "<":
        if value < threshold_value:
            return f"is below the threshold of {threshold_value}{unit}"
        else:
            return f"is at or above the threshold of {threshold_value}{unit}"
    elif comparison == "==":
        if value == threshold_value:
            return f"is equal to the threshold of {threshold_value}{unit}"
        else:
            return f"is not equal to the threshold of {threshold_value}{unit}"
    else:
        return f"is compared to the threshold of {threshold_value}{unit}"


def compose_paragraph_robot_language(readings, thresholds) -> str:
    """
    Robot Language — 로봇끼리 의사소통하는 결정적 기호 기반 발화.
    인간이 직접 이해하지 않아도 되며, sensor data의 구조화된 표현.
    """
    parts = ["[ROBOT]"]
    for r in readings:
        value, unit, stype = _reading_field(r)
        if value is None:
            continue
        rid = getattr(r, "sensor_id", None) or getattr(r, "name", "unknown")
        parts.append(f"{rid}.{stype}={value}{unit}")
    
    # Action 매핑
    conditions = derive_conditions_universal(readings, thresholds)
    for sid, acts in conditions.items():
        for a in acts:
            parts.append(f"action({sid})={a}")
    
    return " ".join(parts)


def compose_paragraph_mixed(readings, thresholds) -> str:
    """
    Robot-Human Common Language — 로봇과 인간이 함께 사용하는 자연어.
    """
    return compose_paragraph_universal(readings, thresholds)
...
def compose_essay_timeseries_multidomain(
    ...
    for domain in domains:
        ...
        # 시계열 추세 분석 (value 기준, 첫 측정 vs 마지막 측정 — 도메인 무관)
        trend = "stable"
        try:
            first_vals = [s.temperature_c for s in history[0] if s.temperature_c is not None]
            last_vals = [s.temperature_c for s in history[-1] if s.temperature_c is not None]
            if first_vals and last_vals:
                delta = last_vals[0] - first_vals[0]
                if abs(delta) > 1.0:
