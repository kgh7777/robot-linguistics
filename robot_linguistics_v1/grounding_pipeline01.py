...
def direction_phrase(value: float, threshold_value: float, comparison: str) -> str:
    """
    측정값과 임계치를 받아 비교 방향이 정확한 자연어 구문 반환.
    ...
    Returns:
        올바른 방향이 반영된 비교 구문 (예: 'is higher than the threshold of 11.0V')
    """
    unit = ""  # 단위는 호출자가 처리
    if comparison == ">=":
        ...
        return f"is at the threshold of {threshold_value}{unit}"
...
def compose_paragraph_universal(readings, thresholds, mode: str = "human") -> str:
    """
    5문장 단락 (도메인 무관, 비교 방향 보강 포함).
    mode='robot': Robot Language (기호 기반)
    mode='human': Robot-Human Common Language (자연어)
    """
    if mode == "robot":
        return compose_paragraph_robot_language(readings, thresholds)
    # mode == 'human' (default)
    conditions = derive_conditions_universal(readings, thresholds)
...
# 교체 후
#                if _check_threshold(value, th):
#                    phrase = direction_phrase(value, th.threshold_value, th.comparison)
#                    parts.append(...)
...
#                    )
                if _check_threshold(value, th):
                    phrase = direction_phrase(
                        value, f"{th.threshold_value}{unit}", th.comparison,
                        is_above_or_below=(th.comparison == ">=")
                    )
                    parts.append(
                        f"The {stype} of {_format_value_with_unit(value, unit)} "
                        f"{phrase}."
...
def compose_essay_timeseries_multidomain(
    ...
    for domain in domains:
        ...
        # 시계열 추세 분석 (온도 기준, 첫 측정 vs 마지막 측정)
        trend = "stable"
        try:
